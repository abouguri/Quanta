...
const INITIAL: AnalyzeState = { phase: 'idle', progress: 0, result: null, error: null, retryAfter: null }
...
export function useAnalyzePort() {
  ...
  const start = useCallback(async (article: ExtractedArticle, language: string = 'en') => {
    setState({ phase: 'measuring', progress: 5, result: null, error: null, retryAfter: null })
    ...
    port.onMessage.addListener((msg: BackgroundToPopup) => {
      if (msg.type === 'PROGRESS') {
        setState(s => ({ ...s, progress: Math.max(s.progress, msg.progress) }))
      } else if (msg.type === 'RESULT') {
        void saveAnalysis(msg.data, article.url)
        setState({ phase: 'result', progress: 100, result: msg.data, error: null, retryAfter: null })
      } else if (msg.type === 'ERROR') {
        if (msg.status === 429) {
          setState({ phase: 'limit', progress: 0, result: null, error: msg.message, retryAfter: msg.retryAfter ?? null })
        } else {
          setState({ phase: 'error', progress: 0, result: null, error: msg.message, retryAfter: null })
        }
      }
    })
